...
You need to have a version of python 3.12+ installed on your machine.
...
You also need the Python dependencies from `requirements.txt`. To install those, run:
```
...
The setup is really straight forward. First, you need to create a new application [here](https://discord.com/developers/applications) and turn it into a Bot-User via the "Bot" menu. Enable Message Content Intent in the bot settings so `!` commands work. Next, you need to add the bot to your server. [Here is a simple explanation on how to do that.](https://github.com/jagrosh/MusicBot/wiki/Adding-Your-Bot-To-Your-Server) Now you need to reveal and copy the token of the bot.

At last, you need to download this repository, copy it to wherever you like, set the `DISCORD_TOKEN` environment variable, and run the startup.bat or startup.sh script, based on your OS. Alternatively, you can run the run.py script with python from the root directory of the project:

```
DISCORD_TOKEN='your_bot_token' python run.py
```

The bot joins a voice channel when a music command is used.
...
```
cd DiscordJockey
DISCORD_TOKEN='your_bot_token' docker compose up --build
```

To run it in the background:

```
DISCORD_TOKEN='your_bot_token' docker compose up -d --build
```
...
If `yt-dlp` prints `Sign in to confirm you're not a bot`, export YouTube cookies from a browser and pass them to the bot.

For a local Python run:

```
DISCORD_TOKEN='your_bot_token' YTDLP_COOKIES_FILE='/path/to/youtube-cookies.txt' python run.py
```

For Docker Compose, save the exported cookies file here:

```
DiscordJockey/cookies/youtube.txt
```
...
The compose file mounts `./cookies/youtube.txt` into the container at `/cookies/youtube.txt`.

Do not commit cookies. They are ignored by `.gitignore`.
